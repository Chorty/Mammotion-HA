...
`0.6.4-beta24` candidate and experimental motion is verified off. Gate 2 passed
...
The host and branch run the still-unaccepted `0.6.4-beta24` candidate. On top of
...
with `rtk_telemetry_stale` when the RTK payload has not changed for 300 s, after
a three-hour-old fix presented as current on 2026-08-07. It does **not** require
RTK to read Fix; that threshold is an open operator decision. A zero-command
...
`pyproject.toml`, `CARD_VERSION` and `uv.lock` (PEP 440 `0.6.4b24`) must always agree, and the
...
suffix `?v=0.6.4-beta24&build=73b17740`. The misleading third-party-map
