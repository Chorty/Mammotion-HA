class CameraAgoraCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot
      ...
      .addEventListener("click", () => this._playVideo());
    this.shadowRoot
      .getElementById("pause-button")
      .addEventListener("click", () => this._stopVideo());
    this.shadowRoot
      .getElementById("fullscreen-button")
      .addEventListener("click", () => this._toggleFullscreen());
    this.shadowRoot
      .getElementById("switch-video-button")
      .addEventListener("click", () => this._switchCamera());
  }
  ...
  async _setupAgoraStream() {
    ...
    try {
      ...
      if (this._config.autostart) {
        ...
        this._playVideo();
      } else this._showLoading("Press play to start video stream");
    } catch (error) {
  ...
  async _playVideo() {
    ...
    try {
      const entityId = this._config.entity;
      const switchVideoButton = this.shadowRoot.getElementById(
        "switch-video-button",
      );
      const videoContainer = this.shadowRoot.getElementById("agora-video");

      // Recover preferred camera (if exists)
      const savedCameraUid = localStorage.getItem(
        "preferredMammotionCameraUid",
      );
      ...
      throw new Error(
        "Direct Agora card retired; use the Mammotion camera entity",
      );
      /*
  ...
  _switchCamera() {
    this._currentVideoIndex =
      (this._currentVideoIndex + 1) % this._remoteUsers.length;
    this._showCurrentVideo();
  }
  ...
  _showCurrentVideo() {
    ...
    if (this._remoteUsers.length > 0) {
      ...
      cameraLabel.textContent =
        this._cameras[currentUser.uid] || `Camera ${currentUser.uid}`;
  ...
  async _stopVideo() {
    ...
    try {
      ...
      const joystickOverlay =
        this.shadowRoot.getElementById("joystick-overlay");

      // Create a temporary container for the elements we want to preserve
      const tempContainer = document.createElement("div");
      if (loadingElement)
        tempContainer.appendChild(loadingElement.cloneNode(true));
      if (joystickOverlay)
        tempContainer.appendChild(joystickOverlay.cloneNode(true));
      ...
      if (this._hass && this._config && this._config.entity) {
        await this._hass.callService("mammotion", "stop_video", {
