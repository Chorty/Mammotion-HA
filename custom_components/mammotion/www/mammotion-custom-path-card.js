...
const CARD_VERSION = "0.6.4-beta115";

// ---- BLE coverage overlay ---------------------------------------------------
// 🔑 Served by the integration itself: `async_setup` registers a StaticPathConfig
// mapping "/mammotion" -> the www dir. This ABSOLUTE path therefore resolves
// whether the card was loaded from /mammotion/ or from /hacsfiles/ -- a relative
// URL would break on one of the two. Generated by
// scripts/build_ble_coverage_map.py and deployed alongside the card.
const BLE_COVERAGE_URL = "/mammotion/ble-coverage.json";
// RSSI is a MAGNITUDE, so it gets one hue light->dark, never a rainbow.
// Stronger signal reads darker.
const BLE_RAMP = [
  "#cde2fb",
  "#9ec5f4",
  "#6da7ec",
...
const BLE_MAX_ZOOM = 8;

// 🚨 The domain is computed PER LAYER, never shared. Two shared-domain versions
// were built and rendered first and both failed: a fixed window, then one domain
// across all layers where a single -93 dBm sample flattened the 724-cell layer
// to one indistinguishable blue. A layer whose cells all hold one value would
// divide by zero, so it gets an arbitrary +/-3 dB window; the colour is
// meaningless there either way, which is why the legend prints the endpoints.
function bleDomainFor(cells) {
  if (!Array.isArray(cells) || !cells.length) return [-90, -55];
  const values = cells
    .map((cell) => Number(cell.rssi))
    .filter((value) => Number.isFinite(value));
  if (!values.length) return [-90, -55];
...
function bleRampColor(rssi, domain) {
  const [lo, hi] = domain;
  const span = hi - lo || 1;
  let t = (Number(rssi) - lo) / span;
  if (!Number.isFinite(t)) t = 0;
  t = Math.min(1, Math.max(0, t));
  return BLE_RAMP[
    Math.min(BLE_RAMP.length - 1, Math.round(t * (BLE_RAMP.length - 1)))
  ];
}

// 🚨 Zoom is applied by moving the SVG viewBox, NOT by a <g transform>.
// `_svgPointFromEvent` maps a click back to map metres via
// `svgEl.getScreenCTM()`, which accounts for the viewBox but NOT for any
// transform on a child group. Zooming with a <g> would leave every click
// landing at its unzoomed position -- waypoints silently placed somewhere the
// operator did not click. The viewBox route keeps the existing hit-testing
// correct with no change to it at all.
function bleZoomedViewBox(W, H, zoom) {
  const k = Math.min(Math.max(Number(zoom?.k) || 1, 1), BLE_MAX_ZOOM);
  const w = W / k;
  const h = H / k;
  // Clamp the centre so the visible window can never leave the drawn map.
  const cx = Math.min(Math.max(Number(zoom?.cx ?? W / 2), w / 2), W - w / 2);
  const cy = Math.min(Math.max(Number(zoom?.cy ?? H / 2), h / 2), H - h / 2);
...
class MammotionCustomPathCard extends HTMLElement {
  constructor() {
    ...
    this._bleOverlay = "";
    this._bleCoverage = null;
    this._bleCoverageError = null;
    this._loadingBleCoverage = false;
    this._bleZoom = { k: 1, cx: null, cy: null };
    this._height = 520;
    this._status =
  ...
  setConfig(config) {
    ...
    try {
      this._bleOverlay = localStorage.getItem(this._bleOverlayKey()) || "";
    } catch {
      this._bleOverlay = "";
    }
  ...
  _bleOverlayKey() {
    return `mammotion-ble-overlay:${this._config?.entity || "default"}`;
  }

  // Lazy, and deliberately failure-tolerant: the overlay is a diagnostic layer,
  // so a missing or malformed asset must never take the map, the gates or Real
  // Go down with it. On failure the overlay switches itself off and says why.
  async _loadBleCoverage() {
    if (this._bleCoverage || this._loadingBleCoverage) return;
    this._loadingBleCoverage = true;
    this._bleCoverageError = null;
    try {
      const response = await fetch(BLE_COVERAGE_URL, { cache: "no-cache" });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      if (!Array.isArray(data?.baseline)) {
        throw new Error("asset has no baseline array");
      }
      this._bleCoverage = data;
    } catch (err) {
      this._bleCoverageError = String(err?.message || err);
      this._bleOverlay = "";
    } finally {
      this._loadingBleCoverage = false;
    }
  ...
  _bleLayers() {
    return [
      { id: "baseline", label: "Aggregate baseline" },
      ...(this._bleCoverage?.proxies || []).map((proxy) => ({
        id: `proxy:${proxy.name}`,
        label: proxy.name,
        proxy,
      })),
  ...
  _bleCellsFor(layerId) {
    const data = this._bleCoverage;
    if (!data || !layerId) return [];
    if (layerId === "baseline") return data.baseline || [];
    if (layerId.startsWith("proxy:")) {
      const name = layerId.slice("proxy:".length);
      return (data.per_proxy || []).filter((cell) => cell.proxy === name);
    }
    return [];
  }

  async _setBleOverlay(value) {
    this._bleOverlay = value || "";
    try {
      if (this._bleOverlay) {
        localStorage.setItem(this._bleOverlayKey(), this._bleOverlay);
      } else {
        localStorage.removeItem(this._bleOverlayKey());
      }
    } catch {
      // Persistence is a convenience; a quota or private-mode failure must not
      // stop the overlay being toggled for this session.
      this._storageWarning = "Could not remember the BLE overlay choice.";
    }
    if (this._bleOverlay) await this._loadBleCoverage();
    this._render();
  }

  _zoomMap(factor, focus = null) {
    const mt = this._mapT;
    if (!mt) return;
    const current = bleZoomedViewBox(mt.W, mt.H, this._bleZoom);
    const k = Math.min(Math.max(current.k * factor, 1), BLE_MAX_ZOOM);
    let { cx, cy } = current;
    if (focus && k !== current.k) {
      // Hold the point under the cursor still while the scale changes.
      cx = focus.x - (focus.x - current.cx) * (current.k / k);
      cy = focus.y - (focus.y - current.cy) * (current.k / k);
    }
    this._bleZoom = { k, cx, cy };
    this._renderMap();
  }
  ...
  _onMapWheel(event) {
    ...
    const mapPoint = this._svgPointFromEvent(event);
    const focus = mapPoint
      ? { x: mt.toSX(mapPoint.x), y: mt.toSY(mapPoint.y) }
      : null;
    this._zoomMap(event.deltaY < 0 ? 1.15 : 1 / 1.15, focus);
  }

  // Cell size is derived from the map transform itself, so the overlay follows
  // the card's own scale and zoom without duplicating any of that arithmetic.
  _renderBleOverlay(svgEl, el, mt) {
    if (!this._bleOverlay || !this._bleCoverage) return;
    const cells = this._bleCellsFor(this._bleOverlay);
    if (!cells.length) return;
    const domain = bleDomainFor(cells);
    const baseline = this._bleOverlay === "baseline";
    const [gridW, gridH] = this._bleCoverage.grid_cell || [1, 1];
    const binM = Number(this._bleCoverage.bin_m) || 1;
    const sizeX = baseline ? Number(gridW) || binM : binM;
    const sizeY = baseline ? Number(gridH) || binM : binM;
    const w = Math.abs(mt.toSX(sizeX) - mt.toSX(0));
    const h = Math.abs(mt.toSY(sizeY) - mt.toSY(0));
    const group = el("g", { "pointer-events": "none" });
    for (const cell of cells) {
      const cx = mt.toSX(Number(cell.x));
      const cy = mt.toSY(Number(cell.y));
      ...
      group.appendChild(
        el("rect", {
          ...
          fill: bleRampColor(cell.rssi, domain),
          "fill-opacity": "0.55",
        }),
      );
    }
  ...
  _mapToolsHtml() {
    const layers = this._bleLayers();
    const options = [
      `<option value=""${this._bleOverlay ? "" : " selected"}>off</option>`,
      ...layers.map(
        (layer) =>
          `<option value="${this._escapeHtml(layer.id)}"${
            this._bleOverlay === layer.id ? " selected" : ""
          }>${this._escapeHtml(layer.label)}</option>`,
      ),
    ].join("");
    const cells = this._bleCellsFor(this._bleOverlay);
    const domain = cells.length ? bleDomainFor(cells) : null;
    let note = "";
    if (this._bleCoverageError) {
      note = `⚠️ coverage unavailable: ${this._escapeHtml(this._bleCoverageError)}`;
    } else if (this._bleOverlay && !cells.length) {
      note = "no samples in this layer yet";
    } else if (domain) {
      const proxyLayer = this._bleOverlay !== "baseline";
      const samples = cells.reduce((sum, c) => sum + (Number(c.n) || 0), 0);
      note =
        `${cells.length} cell${cells.length === 1 ? "" : "s"}, ` +
        `${domain[0]}…${domain[1]} dBm` +
        (proxyLayer
          ? ` · ${samples} sample${samples === 1 ? "" : "s"}`
          : " · aggregate, no proxy attribution");
    }
    const zoomPct = Math.round((Number(this._bleZoom?.k) || 1) * 100);
    return `
        <div class="map-tools">
          <label for="ble-overlay">BLE coverage</label>
          <select id="ble-overlay">${options}</select>
          <span class="ble-note">${note}</span>
          ...
          <button id="map-zoom-reset" type="button" title="Reset zoom and pan">Reset</button>
        </div>`;
  }
  ...
  _renderMap() {
    ...
    const mt = this._mapT;
    if (!mt) return;
    // 🚨 An overlay choice restored from localStorage arrives with no data
    // behind it -- only `_setBleOverlay` fetches, and that only runs when the
    // operator touches the control. Without this, a remembered overlay drew
    // nothing and the toolbar claimed "no samples in this layer yet", which
    // reads as a data problem rather than a load that never happened. The
    // guards (coverage present / in flight / already failed) stop it looping.
    if (
      this._bleOverlay &&
      !this._bleCoverage &&
      !this._loadingBleCoverage &&
      !this._bleCoverageError
    ) {
      this._loadBleCoverage().then(() => this._render());
    }
    while (svgEl.firstChild) svgEl.removeChild(svgEl.firstChild);
    // Zoom rides on the viewBox so `getScreenCTM()` keeps mapping clicks
    // correctly -- see bleZoomedViewBox for why a <g transform> would not.
    // Re-assigning the clamped result normalises out-of-range pan/zoom state.
    ...
    // 🚨 Drawn FIRST, so the overlay sits UNDER the areas, the keep-outs and
    // the path. SVG paints in document order, and a keep-out hidden behind a
    // coverage cell is the exact failure that drove a leg through a trampoline
    // on 2026-08-20. Every cell also carries pointer-events:none, so
    // click-to-add-waypoint works straight through the overlay.
    this._renderBleOverlay(svgEl, el, mt);
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .map-tools { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; padding: 0 12px 8px; font-size: 12px; color: var(--secondary-text-color); }
        .map-tools .tools-spacer { flex: 1 1 8px; }
        .map-tools select { font: inherit; font-size: 12px; padding: 2px 6px; }
        .map-tools button { font: inherit; font-size: 12px; min-width: 32px; min-height: 28px; padding: 2px 8px; }
        .map-tools .zoom-level { min-width: 44px; text-align: center; font-variant-numeric: tabular-nums; }
        .map-tools .ble-note { color: var(--secondary-text-color); opacity: 0.85; }
    ...
    this._q("#ble-overlay")?.addEventListener("change", (event) =>
      this._setBleOverlay(event.target.value),
    );
    this._q("#map-zoom-in")?.addEventListener("click", () =>
      this._zoomMap(1.5),
    );
    this._q("#map-zoom-out")?.addEventListener("click", () =>
      this._zoomMap(1 / 1.5),
    );
    this._q("#map-zoom-reset")?.addEventListener("click", () => {
      this._bleZoom = { k: 1, cx: null, cy: null };
      this._renderMap();
    });
    const svgEl = this._q("#path-map");
