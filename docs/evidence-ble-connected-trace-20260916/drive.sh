...
set -a; . ./.env; set +a
OUT=docs/evidence-ble-connected-trace-20260916
PY=.venv/bin/python
...
for spec in "${legs[@]}"; do
  ...
  $PY scripts/phase1_leg_runner.py $L $TX $TY scored --out $OUT > $OUT/log_$L.txt 2>&1
  rc=$?
  ...
  if (( tries < 4 )) && grep -qE "vio_tracked_features min|ble_client_not_connected" $OUT/log_$L.txt; then
