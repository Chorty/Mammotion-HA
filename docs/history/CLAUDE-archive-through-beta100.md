# CLAUDE.md archive — the running log through beta100 (relocated 2026-09-04)

⚠️ **THIS IS HISTORY. Do not act on any build state, open item, or "next step"
described here.** It is accurate as a *record* — the measurements stand, and
several entries usefully record why an approach was rejected — but it was
written as a rolling log and much of it was already superseded in place.

🔑 **Why it moved.** `CLAUDE.md` had grown to 3,258 lines, of which roughly 250
were live instruction; the rest was this. It loaded into context every session,
and the "keep it, mark it historical" approach had a measured failure rate — the
file itself recorded a paragraph that called an already-shipped fix "NOT
implemented" for thirteen betas and cost a session real work, and a reach bullet
that went stale twice under a "do not re-derive" heading.

**Nothing was deleted.** Every line below is verbatim from `CLAUDE.md` as of
commit `f35131a7`. The live facts that survived the move are in `CLAUDE.md`;
if you find something here that is still true and still load-bearing, promote it
there rather than leaving two copies.

---

# Part 1 — the rolling "Current build" log and its (history) sections
...
# Part 2 — OTA firmware capture (as it read while PAUSED)
...
# Part 3 — build provenance, gate history, beta31/32
