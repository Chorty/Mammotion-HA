# Predeclared (SECOND EDITION): does the corrected facing derivation predict the driven direction on the ground? (2026-09-05)

**Committed before any SCORED run of this series exists.** Written per
CLAUDE.md → "How this project works": the criterion, the falsifier and the abort
rules are fixed here, in advance, so that choosing a rule after seeing which
verdicts it flips is not possible.

Build under test: **beta103** (`4f908b04`, deployed 2026-09-05). Backend
`chorty-0.8.12.post4`.

---

## 0. 🚨 Why there is a second edition, and what it may NOT do

The first edition named **beta102** as the build under test. Two dispatches were
made under it. **Neither produced a scored leg**, and the reason was an
instrument defect, not a result:

Both died on the travel guard — at **344 ms** and **273 ms**, having sent 1 of 11
refresh writes, each reporting `position_sequence_gap` with
`travel_at_trip_m: 0.0` on a healthy link (RTK Fix, BLE −54 dBm, command and stop
both `ok`). The mower travelled 0.0949 m and 0.0578 m against a 0.40 m bound.
Root cause and fix: `4f908b04` — the guard took its sequence baseline from
`handle.latest_position_sample` *after* opening the position stream, so any
payload arriving in that gap made the first drained sample fail the contiguity
check. `max_travel_m` was unusable on every real run.

🔑 **Those two dispatches are VOID on instrument failure, not FAIL.** They are
reported in full and excluded from n. Distinguishing them matters: a FAIL is a
verdict about the control law, and neither dispatch measured the control law at
all. The bearings they did produce (5.43° and 4.40° from prediction) are recorded
in §7.3 as observations and are **explicitly not evidence either way** — over
0.095 m and 0.058 m of travel the position noise floor alone spans ±23° and ±35°.

🛑 **What this edition may NOT do, and does not do:** it does not touch the
criterion, the frozen parameter table, the abort rules or the falsifier. Every
one of those is carried over **verbatim**. The only changes are the build
identity, this section, and §7.3. **If any scored leg had run, no new edition
would have been permissible at all.**
...
*(Carried verbatim from the first edition's amendment, which was itself made
before any leg was dispatched and moved the bar UP, from 3 of 3 to 4 of 4.)*

**The precondition for excluding a re-anchor leg stopped being true before the
series started, so the criterion is 4 of 4.**
...
### 7.3 The two void dispatches, recorded in full

Reported because per-item records are the rule, and because a void run that goes
unrecorded is indistinguishable from a run that never happened.

| | dispatch 1 | dispatch 2 |
| --- | --- | --- |
| trip reason | `position_sequence_gap` | `position_sequence_gap` |
| travel at trip | 0.0 m | 0.0 m |
| window survived | 344 ms of 2400 | 273 ms of 2400 |
| refresh writes | 1 of 11 | 1 of 11 |
| actual travel | 0.0949 m | 0.0578 m |
| predicted bearing | 279.336° | 279.203° |
| measured bearing | 284.768° | 283.604° |
| difference | 5.43° | 4.40° |
| noise floor at that travel | ±22.9° | ±34.7° |

🛑 **The last three rows must not be read as support for the model.** Both
differences sit far inside the noise floor of their own displacement; they would
look the same if the model were wrong by 20°. They are recorded to show the runs
happened and produced nothing, not to bank a result the series has not earned.
...
- ⚠️ **A travel-guard trip is NOT in this list and never was.** It is an
  instrument fault: the run is VOID and re-run, not scored. If the guard trips
  **twice more** on legs of this series, the series aborts — `4f908b04` is then
  not the whole story and continuing would be guessing, which is what the first
  edition's two dispatches already cost.
